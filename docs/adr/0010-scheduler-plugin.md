...
**P10-fix-002 + P11-T-007 + P11-fix-004/005 NRT CRD + RBAC + default partial close(2026-05-21 → 2026-05-22)**:
...
- **P11-T-007**(`7e8f026` · ADR-0017 §2 Decision D 5th 优先级):**Approach B vendored CRD YAML**(per phase11-plan §3 P11-T-007 default · 不引入 noderesourcetopology-api subchart · chart 自管 release cadence)· `deploy/helm-charts/scheduler-plugin/crds/noderesourcetopologies.yaml`(从 `sigs.k8s.io/scheduler-plugins@v0.34.7/manifests/noderesourcetopology/crd.yaml` vendored · v1alpha2 only)· helm 自动 pre-install hook 安装 CRD · `numaAffinity.enabled: true` default 恢复。
- **P11-fix-004**(`25f4d64`):phase-11-complete tag push 后 CI gate red · scheduler-plugin Pod NotReady · 因 ClusterRole 缺 `topology.node.k8s.io.noderesourcetopologies` get/list/watch verbs · SA 拿不到 NRT 读权限 → nrt.New 403 forbidden。修法:templates/rbac.yaml ClusterRole 加 conditional `{{- if .Values.numaAffinity.enabled }}` block 含 NRT verbs。
- **P11-fix-005**(本 commit):fix-004 后 CI 仍 red · nrt.New informer cache sync 在 kind 1.34.3 cluster > 180s default helm install --wait timeout(实测 phase6 install 仍 context deadline exceeded)· 反复 RBAC / CRD 调试不再 incremental 收益。**默认 revert false**(同 P10-fix-002 working state)· 完整基础设施 ready(CRD + RBAC + ConfigMap 全 ship)· operators opt in 路径:`helm install --set numaAffinity.enabled=true --timeout=300s`(给 nrt.New cache sync 时间)。
- **known-issues #12 partial close trail**:(1) Phase 6 NumaAffinity placeholder → (2) Phase 8/9 baseline bump 等待 → (3) P10-T-005 wrap body land → (4) P10-fix-002 default false 临时禁 → (5) P11-T-007 NRT CRD bundle ship → (6) P11-fix-004 NRT RBAC ship → (7) P11-fix-005 default 维持 false + 完整 opt-in 基础设施 ready。**完整 default true land 留 Phase 12+ K8s 1.36+ baseline bump cohort + helm --timeout bump · 与 Partitionable Devices Track C 联动**。本 cycle 算 partial close(infrastructure 全 ship · 默认仍 conservative)。
