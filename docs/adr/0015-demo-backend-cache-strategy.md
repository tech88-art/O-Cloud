...
**Status update(2026-05-22 P11-T-003 · ADR-0017 §2 Decision D 1st 优先级)**:**chart packaging + main.go leader-elect wire LANDED**。`deploy/helm-charts/demo-backend/` 9 file 全 ship(Chart.yaml + values.yaml + templates/_helpers.tpl + deployment.yaml + service.yaml + configmap.yaml + serviceaccount.yaml + rbac.yaml + servicemonitor.yaml + .helmignore)· `backend/pkg/cache/leaderelect.go` 加 client-go `tools/leaderelection` runner(`RunLeaderElection` + `ConfigFromOptions` helper)· `backend/cmd/demo-backend/main.go` 加 `cfg.Lease.Enabled` gate + Singleton 构造 + goroutine launch · `backend/pkg/config/config.go` 加 `LeaseConfig` 结构 + Viper env var binding(`OCEDGE_LEASE_*`)· `backend/pkg/api/router.go` Handler.CacheSingleton 字段加。Lease RBAC = namespaced Role + RoleBinding 而非 ClusterRole(scoped to `leaseElection.namespace` default `ocloud-system`)· coordination.k8s.io/v1 Lease verbs `get/list/watch/create/update/patch/delete` · POD_NAME downward API → HolderIdentity。验证:`go build ./...` clean · `go test ./pkg/cache/... ./pkg/config/...` PASS · `helm lint --strict` clean · `helm template` 9 kind renders OK · `helm template --set serviceMonitor.enabled=true` ServiceMonitor renders。`backend/docs/cache.md` 新增 §3 chart wiring 文档(Pod lifecycle T+0 → T+shutdown 流程图 + RBAC scope + Phase 12+ 待办)。kind smoke leverage T107 phase11/ folder。**实现路径选择**:client-go `tools/leaderelection` 直接 wire 而非引入 `controller-runtime`(backend 已有 client-go 间接依赖 · 不引入 controller-runtime 新包 · 与 backend "lightweight Gin process" 设计 align · 与 ADR-0015 §3.1 "0 new external dependency" 文字略有偏离 — 实际新增 `k8s.io/client-go/tools/leaderelection/resourcelock` 子包但属 client-go 已存在依赖树 · 不破 go.mod 直接依赖)。
