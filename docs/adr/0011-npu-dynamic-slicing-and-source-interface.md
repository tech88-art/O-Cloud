...
| Phase 12 P12-T-101 (2026-06-01) | Phase 12 = 构建 target 反转 ≠ 真硬件验证(ADR-0019 §2 Decision C) | deferred Phase 13 | 6th carry |
| **Phase 13 P13-T-002 (2026-06-02)** | **lab access available**(ad-hoc materialise) | **🟢 ACTIVATED — lab-gating 翻转 light-up · real-ascend body land(ADR-0024)** | **flip · 6-phase(P7-P12)defer 期结束** |
...
> 🟢 **2026-06-02 LAB ACTIVATED (P13-T-002 · §3 policy default-defer FLIP · supersede by ADR-0024)**:Phase 12 P12-T-101 命中 6th carry(Phase 12 spine = 构建 target 反转 ≠ 真硬件验证 · per ADR-0019 §2 Decision C)。**Phase 13 entry(2026-06-02)用户 lab-ready 信号** → **ADR-0016 §2 Decision B trigger 2("ad-hoc lab access materialise")FIRED** → **lab-gating default-defer 翻转 light-up**:`real-ascend` Source 从 stub(`ErrNotImplemented`)→ 真体 · 6-phase(P7-P12)default-defer 期结束 · Bucket B 真硬件 body(real-Ascend + 真 telemetry + 真拓扑 + 真 Deploy + 真推理)成为 Phase 13 deliverable(非 carry)。**架构详 ADR-0024**(Decision A-G · supersede 本 §3 default outcome)。**policy *机制* 不删**(P2 边界):default sourceType=mock-json + real-ascend opt-in + CI 不依赖 lab 全保留(demo 版仍走 mock-json)· 翻转的是 real 版 *default outcome*(defer → light-up)· 非删 mock 路径(ADR-0024 §2 Decision A/G)。**验证分工**:功能正确性由 demo 持续验(captured-fixture · CI 常驻)· 真机只验「对接」(real Source 读真 npu-smi/DCMI + 输出同形)· 真硬件单点 block → 交付 body + 标 lab-driver-gated · 不开 Phase 14(ADR-0024 §3 fallback)。
