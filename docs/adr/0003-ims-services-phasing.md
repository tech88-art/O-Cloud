...
- **IMS-1 node-lifecycle-operator** P11-T-004 commit `aedb1a0`:chart `deploy/helm-charts/node-lifecycle-operator/` 8 file + `operators/node-lifecycle-operator/cmd/main.go` controller-runtime manager wire + `internal/controller/reconciler.go` ctrl.Reconciler shell + `api/v1alpha1/groupversion_info.go` SchemeBuilder + AddToScheme + Dockerfile + go.mod controller-runtime/client-go added · `go build ./...` clean + `go test -vet=off ./...` PASS + `helm lint --strict` clean + `helm template` 7 kind render OK
- **IMS-2 software-mgmt-operator** P11-T-005 commit `c5998d6`:同 IMS-1 pattern · chart `deploy/helm-charts/software-mgmt-operator/` 8 file + cmd/main.go SoftwareBundleReconciler register + reconciler.go shell + SchemeBuilder + Dockerfile + go.mod tidy。RBAC 差异:SoftwareBundle controller 仅 list/watch Nodes(不 patch)· 真 rollout actuation 由 per-node agents 完成(Phase 12+ NodeSoftwareBundleStatus CR)· 当前 Phase 11 minimum-viable observation 走 annotations(`applied/in-progress/failed-nodes`)。`go build ./...` clean + `go test -vet=off ./...` PASS · `helm lint --strict` clean + `helm template` 7 kind render OK
- **IMS-3 bare-metal-provisioning-operator** P11-T-006(本 commit):chart `deploy/helm-charts/bare-metal-provisioning-operator/` 8 file + cmd/main.go BareMetalNodeReconciler register + reconciler.go shell(含 `resolveCredentials` 拉 Secret + `buildBMC` address-scheme 分发)+ `internal/client/redfish.go` + `internal/client/ipmi.go` 两 Phase 11 stub(in-memory PowerState · 真 Redfish HTTPS / `ipmitool` shell-out 留 Phase 12+)+ SchemeBuilder + Dockerfile + go.mod tidy。RBAC 差异:替换 Node patch verbs 为 **Secret read**(BMC credentials 通过 `Spec.BMC.CredentialsRef` Secret 解析 username/password)。values.yaml 加 BMC supportedSchemes(redfish:// + redfish+https:// + ipmi://)+ Spec/Secret 示例文档。`go build ./...` clean + `go test -vet=off ./...` PASS(5 packages 含新 internal/client noop)· `helm lint --strict` clean + `helm template` 7 kind render OK
