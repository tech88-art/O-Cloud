...
- **IMS-2 software-mgmt-operator** P11-T-005(本 commit):同 IMS-1 pattern · chart `deploy/helm-charts/software-mgmt-operator/` 8 file + cmd/main.go SoftwareBundleReconciler register + reconciler.go shell + SchemeBuilder + Dockerfile + go.mod tidy。RBAC 差异:SoftwareBundle controller 仅 list/watch Nodes(不 patch)· 真 rollout actuation 由 per-node agents 完成(Phase 12+ NodeSoftwareBundleStatus CR)· 当前 Phase 11 minimum-viable observation 走 annotations(`applied/in-progress/failed-nodes`)。`go build ./...` clean + `go test -vet=off ./...` PASS · `helm lint --strict` clean + `helm template` 7 kind render OK
