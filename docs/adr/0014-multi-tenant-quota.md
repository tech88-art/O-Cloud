...
> 🆕 **Phase 11 T104 LANDED**:`operators/inference-operator/api/v1alpha1/clusterquota_types.go` 加 ClusterQuota / ClusterQuotaList / ClusterQuotaSpec / ClusterQuotaStatus / ClusterQuotaUsage 5 type · 复用 namespace-scope Quota 的 QuotaEnforcement + QuotaUsage 子 struct(无 schema duplication)· `ClusterQuotaUsage.RecomputeTotal()` method 把 PerCluster map sum 进 Total · SchemeBuilder.Register 自动注册。chart `deploy/helm-charts/inference-operator/crds/inference.ocloud.edge.example.com_clusterquotas.yaml`(controller-gen 生成 · helm pre-install hook 自动 apply)。3 unit tests(JSONRoundtrip + UsageAggregation + UsageAggregationEmpty)。admission webhook B(NPUVerticalScaler scale rate)+ Quota controller cross-cluster usage tick reconcile · 留 后续 task / Phase 12+ 完善(本 task ship CRD schema + aggregation primitive + cross-ref propagation policy YAML)。
