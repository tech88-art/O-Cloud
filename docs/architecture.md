...
| **M6 平台真实化 + 操作台一体化** | Phase 12 | aarch64 鲲鹏(Kunpeng 920)+ openEuler 真实目标平台(supersede ADR-0001 §13 · 华为 Atlas 800 原生配置)+ 拓扑数据全保真(NPU PCIE + node↔node network 边 + npu↔npu HCCS 边 + workload→node 边 + 带宽属性)+ 前端 one-page workspace(吸收 workloads/deploy/metrics/logs 四页)(per ADR-0019 §2 Decision B M6 naming reset · 原 production-hardening cohort 顺延 Phase 13+) |
| **M7 真生产化收口**(final · **landed phase-13-complete 2026-06-03** · 软件层完整 · 真机实测 lab-gated) | Phase 13 | real 版完整化 + 真硬件点亮 → 项目收口:Bucket B 真硬件 body(real-Ascend Source npu-smi/DCMI + 真 telemetry + 真拓扑聚合 GetTopology + 真 Deploy() + 真 CANN/vllm-ascend PD 推理 · lab-gating 翻转 supersede ADR-0011 §3)+ Bucket A 生产硬化(OIDC/RBAC + Vault secret + ClusterQuota webhook B 真强制 + Karmada HA + vLLM PD P99 SLA)(per ADR-0023 §2 entry decisions · M7 final · **不规划 Phase 14** · 4 carry 移出核心交付 · demo = 长期功能验证台) |
...
| Phase 7 (动态切分) | 突破硬模板需要驱动层能力 | 与昇腾团队交互；准备 fallback：多模板组合（**Phase 7 P7-T-001 落 ADR-0011 提交 fallback 作为 deliverable + Source 接口抽象 + lab gating 政策**）· **Phase 13 lab 激活**(ADR-0024 · 2026-06-02)：`real-ascend` Source 真体 light-up(npu-smi `info`/`info -t topo` via `npusmi.ParseTopoMatrix` + DCMI health)· supersede ADR-0011 §3 6-phase default-defer · 详 ADR-0024 §2 Decision A/B |
...
| Phase 9 (O2 DMS) | O-RAN O2 规范持续演进 | **landed phase-9-complete (2026-05-21)** — 16 task chain(28e660e..T108)· 2 ADRs (0013 O2 DMS + 0014 Quota) + Quota CRD types + controller + 2 ValidatingAdmissionWebhooks + O2 DMS adapter scaffold + body (21 tests) + PromQL custom metric extension + propagation polish + IMS 3 scaffold (api types only · Phase 10 controller body) + demo-backend cache spike(ADR-0015 draft for Phase 10) + kind smoke E2E ext。3 W2 gating outcomes: T101 Volcano deferred default · T102 NumaAffinity auto-deferred per T003 doc-only · T106 Source.RealAscend 3rd carry。K8s baseline stays 1.32 per P9-T-003 doc-only refresh(framework migration deferred to Phase 10 三件套 task chain)。详 `docs/checkpoint-phase9.md` §6 Phase 10 handoff brief。 |
...
| Phase 12 | 平台真实化 + 操作台一体化(arch §1.3 M6 milestone · Track A aarch64 鲲鹏 + openEuler 真实目标平台 supersede ADR-0001 §13 · Track B 拓扑数据全保真 · Track C 前端 one-page workspace 吸收 4 页) | **landed phase-12-complete (2026-06-01)** — 16/16 task chain(4 W1 docs/contract gate + 5 W2 Track A 平台化 ∥ Track B 后端数据 + 5 W3 Track C 前端 one-page + 2 Closer)· **4 entry ADRs**:ADR-0019(entry · spine = 平台真实化 + 拓扑全保真 + one-page UI · milestone M6 naming reset · production-hardening cohort → Phase 13+ · 3 carry tracks 维持 default-defer)+ ADR-0020(aarch64 鲲鹏 + openEuler target · supersede ADR-0001 §13)+ ADR-0021(拓扑契约全保真扩展 · 本 phase 唯一 api-contract RFC)+ ADR-0022(one-page UI IA)。**3 active carry tracks 维持 default-defer**(Track A lab 6th / Track B Volcano 4th / Track C Partitionable 3rd · 与 Phase 12 scope 无 direct dependency · Phase 12 Track A 平台化 = 构建 target 反转 ≠ 真硬件验证 · 真 aarch64 lab 运行验证留 Phase 13+)。**Outcome**:3 track 全 land(平台 target 反转 multi-arch buildx + helm arm64 渲染 + install.sh openEuler · 拓扑全保真 network/hccs/runs-on/PCIE · 前端单页吸收 4 页 Splitter + 右栏 dispatch + preset bar)· 前端 80 unit + 9 e2e + 11 render-verify 截图 · render-verify 抓修 3 真 bug(canvas balloon / 边 handle 缺失 / 右栏 stale)。production-hardening cohort 顺延 Phase 13+。详 `docs/checkpoint-phase12.md` + `docs/phase12-plan.md` §1-§9 + ADR-0019..0022 |
| Phase 13 | 真生产化收口(arch §1.3 M7 final milestone · real 版完整化 + 真硬件点亮 → 项目收口 · Bucket B 真硬件 body + Bucket A 生产硬化 · 不规划 Phase 14) | **landed phase-13-complete (2026-06-03)** — 16/16 task · M7 final milestone · 项目收官(软件层完整 · demo/real 隔离闭合 · 真机实测 lab-gated · harness `tests/e2e/real/` 就位)。**3 entry ADRs**:ADR-0023(entry · spine = real 版完整化 + 真硬件点亮 → 项目收口 · M7 final · 不规划 Phase 14 · lab-gating 翻转 · Bucket A/B scope · 4 carry 移出核心交付 · demo = 长期功能验证台)+ ADR-0024(真硬件 lab 激活 + Bucket B 架构 · supersede ADR-0011 §3 lab-gating default-defer)+ ADR-0025(生产硬化架构 Bucket A · → build-doc §5 清单逐项 close)。**Bucket B 真硬件 body**(T101-T105):real-Ascend Source 真体(npu-smi/DCMI · 复用 npusmi/parse.go)+ 真 NPU telemetry(exporter DCMI/npu-smi 真读 · simulator off)+ backend 真拓扑聚合(k8s/crd GetTopology 替换 ErrCapabilityUnavailable)+ 真 Deploy()/DeleteDeploy()(k8s source apply)+ 真 CANN + vllm-ascend PD 分离推理。**Bucket A 生产硬化**(T201-T206):OIDC/TokenReview validator 真体 + middleware wiring + Dex 参考 IdP + Vault/external-secrets + ClusterQuota webhook B 真强制 + Karmada control-plane HA + vLLM PD P99 SLA harness。**验证 posture**(plan §8):功能正确性由 demo 持续验(CI 常驻)· 真机只验「对接」(real Source 读真 npu-smi/DCMI + 输出同形)· 真硬件单点 block → 交付 body + 标 lab-driver-gated · 不开 Phase 14。详 `docs/phase13-plan.md` §1-§9 + ADR-0023..0025 |
...
| Phase 12 | **拓扑数据全保真扩展**(NPU `pcieBandwidthGBps` host↔NPU 带宽 + emit `network` node↔node 边 + emit `hccs` npu↔npu 边 + 新 `runs-on` 非 NPU workload→node 边 + edge `bandwidthGBps/medium/utilization` 属性供 hover · 单位 GB/s 同轴) | P12-T-003 ADR-0021 + api-contract.yaml 扩 + gen:types regen(swagger-cli `is valid`)· P12-T-104 aggregator emit · P12-T-105 mock fixtures + schema.json | ADR-0021(承 ADR-0004 + ADR-0005 + ADR-0006) |
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| 昇腾 910B 真机访问受限 | Phase 5+ 卡住 | Phase 1-4 全部基于 Mock + 仿真，硬件就位再切;Phase 7-9 实战 3 prior defers per ADR-0011 §3 default policy(no signal → defer)· Phase 10 T102 4th attempt + 5th carry posture re-eval triggers codified in ADR-0016 §2 Decision B;synthetic ring fixture(`set-b-multi-ring` · Phase 7+8+9+10 cumulative)cover CI + demo flow 主干 · 真硬件 stamp 价值是 *verification* 而非 *deliverable substrate* · **🟢 RESOLVED Phase 13(2026-06-02)**:lab access materialise → ADR-0024 翻转 lab-gating(6-phase P7-P12 default-defer 期结束)· Bucket B 真硬件 body light-up · 验证分工 = demo 验功能 / 真机验对接(plan §8 + ADR-0024 §3) |
