...
| Phase 9 | 多站点 demo backend 缓存重构(LRU 进程内 → Redis/singleton/stateless) | **spike landed via P9-T-107 (2026-05-21)** — `docs/research/demo-backend-cache-spike.md` 评估 3 路径 + Karmada cross-cluster cache coherence + ADR-0015 draft outline(推荐 §3.3 singleton with active-active failover · 保留 in-process LRU + K8s Lease leader-elect · 0 new external dependency)· Phase 10 W1 entry 起草 ADR-0015 直接 pick up |
