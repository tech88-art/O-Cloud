...
| Phase 12 | 平台真实化 + 操作台一体化(arch §1.3 M6 milestone · Track A aarch64 鲲鹏 + openEuler 真实目标平台 supersede ADR-0001 §13 · Track B 拓扑数据全保真 · Track C 前端 one-page workspace 吸收 4 页) | **in flight via P12-T-001..T302 (2026-06-01 起)** — 16 task chain(4 W1 docs/contract gate + 5 W2 Track A 平台化 ∥ Track B 后端数据 + 5 W3 Track C 前端 one-page + 2 Closer)· **4 entry ADRs**:ADR-0019(entry · spine = 平台真实化 + 拓扑全保真 + one-page UI · milestone M6 naming reset · production-hardening cohort → Phase 13+ · 3 carry tracks 维持 default-defer)+ ADR-0020(aarch64 鲲鹏 + openEuler target · supersede ADR-0001 §13)+ ADR-0021(拓扑契约全保真扩展 · 本 phase 唯一 api-contract RFC)+ ADR-0022(one-page UI IA)。**3 active carry tracks 维持 default-defer**(Track A lab 6th / Track B Volcano 4th / Track C Partitionable 3rd · 与 Phase 12 scope 无 direct dependency · Phase 12 Track A 平台化 = 构建 target 反转 ≠ 真硬件验证 · 真 aarch64 lab 运行验证留 Phase 13+)。详 `docs/phase12-plan.md` §1-§9 + ADR-0019 §1-§5 |
