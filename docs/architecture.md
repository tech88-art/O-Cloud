...
| Phase 10 | 真实硬件对接 + 演示打磨(arch §1.3 Phase 10 row · M4 工程化对外 closer · 主线 2 真硬件 + 主线 3 完整 multi-pool / multi-tenant 演示) | **in flight via T102+T201 + ADR-0016 fallback path (2026-05-21 · P10-T-002)** — `docs/adr/0016-lab-onboarding-and-phase-11-outlook.md` Accepted · codify 4 things:(a) T102 4th attempt detailed gating policy(沿用 ADR-0011 §3 default · no signal → defer · `[LAB-CONDITIONAL]` subagent brief prefix)·(b) 5th carry to Phase 11+ rationale + 3 distinct posture re-evaluation triggers(Phase 11+ entry meeting / ad-hoc lab signal / M5+ milestone reset)·(c) T201 真硬件 multi-pool / multi-tenant 演示打磨 fallback path detail — 路径 P 真硬件 100% deliverable / 路径 F synthetic ring fallback 80% landed(20% 缺真硬件 stamp · checkpoint § Phase 10 row 标 "with synthetic ring fallback")·(d) §3 Phase 11+ 8 candidate streams enumeration(真 multi-site + live migration + fabric switch + production SLA + OIDC IdP 完整 + KEP-4815 GA wait + Volcano gang-scheduling + O2 IMS R1 v05.00+ migration · 与 checkpoint-phase9 §6 10 Phase 10 candidate workstreams 不重叠)· §4 3 Open questions(lab posture re-eval timing / Phase 11+ primary spine / M5 milestone naming)· 详 ADR-0016 §1-§5 |
