...
| Phase 11 | 真生产化 foundation(arch §1.3 后续 M5 milestone foundation · 主线 1 chart packaging spine 闭环 + 主线 2 Karmada propagation 第一波 production-grade + 主线 3 Frontend src/ Workload page extension) | **in flight via P11-T-001..T204 (2026-05-22 起)** — 20 main task + 4 Frontend UX sub-track per `docs/phase11-plan.md` §2 task package overview · P11-T-001 起手 = ADR-0017 entry decisions(本 commit)· **Spine A 真生产化 foundation subset**(不含完整 P99 SLA / 完整 OIDC IdP / 真多机房 · 留 Phase 12+)· **M5 真生产化 foundation milestone**(foundation 后缀明示不 over-promise · Phase 12+/M6 在此基础加 production hardening cohort)· **Lab gating 5th attempt default-defer 维持**(ADR-0011 §3 policy core 不修改 · ADR-0016 §2 Decision B trigger 1 W1 entry meeting fire + trigger 3 M5+ milestone reset 保留 Phase 12+)· **Chart packaging spine 优先级**:T003 demo-backend first → T004/T005/T006 3 IMS parallel-eligible → T007 NRT bundle 独立 → T008 inference-operator chart edit 最简单 last(详 ADR-0017 §2 Decision D)· **Karmada propagation 第一波 子契约**:T002 ADR-0018(host + 2 member kind cluster minimum 单机模拟 multi-site)· post-tag CI gate(per memory `feedback_post_tag_ci_gate.md`)Phase 11 W3 T204 后 watch dev HEAD CI · 详 `docs/adr/0017-phase-11-entry-decisions.md` + `docs/phase11-plan.md` |
