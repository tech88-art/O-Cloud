...
func SetCondition(conds *[]metav1.Condition, c metav1.Condition) {
	...
	for i := range *conds {
		...
		if existing.Status == c.Status {
			// preserve transition time when status hasn't flipped
...
// It returns true when a deletion happened.
...
// MakeOwnerRef builds a controller-style OwnerReference for obj.
// Controller and BlockOwnerDeletion are both set to true so child resources are
// cleaned up via Kubernetes garbage collection when the owner is deleted.
