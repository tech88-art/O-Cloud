...
**当前阶段**:**Phase 10 complete — M4 工程化对外 milestone CLOSER**(2026-05-21 · tag `phase-10-complete`)· 20 task chain T001-T204。**3 三件套(K8s 1.34 baseline + framework migration + NumaAffinity wrap)**:T003 K8s 1.32→1.34.3 lockstep · T004 framework migration(9 files NodeInfo/CycleState struct→interface · `k8s.io/kube-scheduler/framework` data contract)· T005 NumaAffinity wrap body(sched-plugins v0.34.7 nrt.New delegate · chart default ENABLED · **known-issues #12 RESOLVED · 5-phase carry closer**)。**3 IMS controller body 全 land**:T007 IMS-1 node-lifecycle(12 transitions · 8 states · 14 tests)+ T008 IMS-2 software-mgmt(3 rollout strategies · 15 tests)+ T101 IMS-3 bare-metal-provisioning(13 transitions · 7 ProvisioningStates · 17 tests)· 3 pure-Go Reconcile substrates · chart packaging deferred Phase 11+ stream。**ADR forward note polish**:T103 O2 DMS authn(3 Validator interface · 10 tests)+ T104 Quota token-bucket rate algorithm(6 tests)+ T106 ProxyImage chart flip(EffectiveProxyImage helper · 4 tests · **known-issues #13 RESOLVED · 5-phase carry closer**)。**3 docs ADRs**:ADR-0015 demo-backend cache strategy(singleton + K8s Lease)+ ADR-0016 lab onboarding 4th attempt + Phase 11+ 8 candidate streams · 3 Open questions。**真硬件演示打磨**:T201 master-demo.sh 12-step orchestrator · synthetic ring fallback path per ADR-0016 §2 Decision C · 80% landed(20% 缺真硬件 stamp · T102 5th carry)。**3 deferred outcomes**:T102 LAB-CONDITIONAL 5th carry + T108 Volcano 2nd defer + T202 Partitionable Devices defer(全 default policy 无 user signal)。详 `docs/checkpoint-phase10.md` §6 Phase 11+ handoff brief。
...
**下一阶段**:**Phase 11+ chart packaging stream**(per ADR-0016 §3 真生产化 spine · 4 chart 联动:IMS-1/2/3 + demo-backend · cmd/main.go controller-runtime manager wire · cert-manager / RBAC / Dockerfile / envtest 真集群 verify)+ Phase 11+ candidate workstreams 8 enum(per ADR-0016 §3 · 真 multi-site + live migration + fabric switch + production SLA + OIDC IdP 完整 + KEP-4815 GA wait + Volcano gang + O2 IMS v05.00+ migration)· 详 `docs/checkpoint-phase10.md` §6 + ADR-0016 §3。
