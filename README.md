...
**当前阶段**:Phase 8 complete — Busy-idle 垂直伸缩 (重启切片 pattern per ADR-0012) + NPUVerticalScaler CRD + 8-step Reconcile + PrometheusIngestor / FakeIngestor + AllocateBundle 控制器 wiring (T105-v2 · Phase 7 deferred body) + HCCS placement hard-fail 升级 (T104-v2) + Volcano spike (Phase 9 entry decision matrix) — **完成**(15/15 statuses: 9 net-new + 4 doc-only fallback per gating + 2 deferred · tag `phase-8-complete`)。**Mutation model adaptation**:NPUVerticalScaler patches `ModelService.metadata.annotations[npu.huawei.com/slice-template]`(spec.template.sliceTemplate 字段实际不存在 in ModelService schema)。**User stay K8s 1.32 决策**(chat 2026-05-21)blocks 3 Phase 7 deferred items(T003 NumaAffinity wrap upgrade · T004 ProxyImage chart flip · T101+T102 Partitionable Devices Beta)— 全部顺延 Phase 9-10 per plan §6 fallback + upstream sched-plugins / kindest/node lag。Lab-gating outcome:T105 Source.RealAscend body deferred Phase 10 per ADR-0011 §3 default(no lab signal)。

**上一阶段**:Phase 7 complete — NPU 动态切分 (多模板组合 fallback per ADR-0011) + Source 接口抽象 (mockjson + realascend stub + factory dispatch) + NPUSliceTemplate CRD + template engine + reconciler + AllocateBundle 函数 + HCCS adjacency 8-card 默认 + schedulerName auto-stamp (closes known-issues #11) + npu-smi parser scaffold + kind smoke ext + Partitionable Devices spike — 完成(15/15 statuses: 13 net-new + 2 doc-only fallback per gating + 1 lab-deferred · tag `phase-7-complete`)。
**早期阶段**:Phase 1(核心样机 + Mock 数据)44/44 · Phase 2(真实数据源切入)15/15 · Phase 3(pool controllers + ascend-npu-exporter-plus + ADR-0008 PD Router design)15/15 · Phase 4(npu-dra-driver scaffold + ADR-0001 v3 + CANN matrix)15/15 · Phase 5(real claim allocation + NPUSliceAllocation CRD + inference-operator controller body + PD Router mutating webhook + cert-manager wiring)15/15 · Phase 6(HCCS/NUMA-aware scheduler-plugin + pool-operator HCCS topology + inference-operator metrics + vllm-ascend PD proxy_server)15/15 · 全部完成
**下一阶段**:Phase 9(K8s baseline bump re-eval · Karmada multi-site federation · multi-tenant quota controller · O2 DMS adapter · Volcano gang-scheduling implementation · PromQL custom metric extension · 详见 `docs/checkpoint-phase8.md §6` handoff)
