...
**当前阶段**:Phase 9 complete — M4 工程化对外 milestone · **O2 DMS Adapter** (K8s Profile NB per ADR-0013 · scaffold + body · 21 tests · O-RAN.WG6.O2IMS-INTERFACE-R003-v04.00 lock) + **Multi-tenant Quota CRD + admission webhook** (ADR-0014 · controller + 2 ValidatingAdmissionWebhooks · 10 tests) + PromQL custom metric extension (ADR-0012 §7 landed) + deployment_builder + claim_builder annotation propagation polish (removes phase8/install.sh cmd_demo_bundle_path workaround) + **IMS 7 服务剩 3 项 scaffold** (node-lifecycle + software-mgmt + bare-metal-provisioning · api types only per CLAUDE.md §14.2 · 9 tests) + demo-backend cache spike (ADR-0015 draft outline for Phase 10) + Phase 9 kind smoke E2E ext — **完成**(16/16 statuses: 9 net-new + 5 doc-only fallback + 2 deferred-gated · tag `phase-9-complete`)。**3 W2 gating outcomes**: T101 Volcano deferred per default(no training-job signal)· T102 NumaAffinity auto-deferred per T003 doc-only refresh outcome(framework drift)· T106 Source.RealAscend 3rd carry per ADR-0011 §3 default(no lab signal)。**K8s baseline stays 1.32** per P9-T-003 doc-only — bump 1.34 attempted but framework migration (NodeInfo/CycleState interface) exceeded T003 Forbidden Paths · Phase 10 carry as "baseline bump + framework migration + NumaAffinity wrap" 三件套 task chain。详 `docs/checkpoint-phase9.md` §6 Phase 10 handoff brief。

**上一阶段**:Phase 8 complete — Busy-idle 垂直伸缩 (重启切片 pattern per ADR-0012) + NPUVerticalScaler CRD + 8-step Reconcile + PrometheusIngestor / FakeIngestor + AllocateBundle 控制器 wiring (T105-v2 · Phase 7 deferred body) + HCCS placement hard-fail 升级 (T104-v2) + Volcano spike (Phase 9 entry decision matrix) — 完成(15/15 statuses: 9 net-new + 4 doc-only fallback per gating + 2 deferred · tag `phase-8-complete`)。
...
**下一阶段**:Phase 10(真实硬件对接 + 完整 multi-pool/multi-tenant 演示打磨 · IMS 3 controller body + reconcile loops + helm charts · demo-backend cache singleton-with-failover impl per ADR-0015 · K8s baseline bump + framework migration + NumaAffinity wrap 三件套 · O2 DMS Phase 10 polish (authn/z + Karmada + subscription + lifecycleOperation persistent + R004 upgrade evaluate) · Quota Phase 10 polish (ClusterQuota + Karmada propagation + strong-consistency + token-bucket + frontend) · 详见 `docs/checkpoint-phase9.md §6` handoff)
